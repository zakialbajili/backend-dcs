...
class _FGstockOpname {
  ...
  listFGstockOpname = async ({ supplierId: supplierId, month: month }) => {
    try {
      if (supplierId != undefined && month == undefined) {
      ...
      } else if (supplierId == undefined && month != undefined) {
        ...
        const filteredList = listFGstockOpname.data.filter(function (item) {
          const monthItem = item.created_at.substring(5, 7);
          return monthItem == month && item.supplierId == supplierId;
        });
        return {
          status: true,
          code: 200,
